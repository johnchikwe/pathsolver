...
class GUI:
    ...
    def get_end_point(self):
        ...
        while self._start_point == self._end_point or self.actual_board[self._end_point[0]][self._end_point[1]] == 'ocean':
            end_row = random.randint(0, height - 1)
            self._end_point = (end_row, random.randint(0, len(self.board[end_row]) - 1))

    # will load images in the global dictionary
    # should be called only once as this operation is expensive
    @classmethod
    def load_images(self):
        IMAGES['grass'] = pg.transform.scale(pg.image.load('./images/grass.png'), (IMAGE_SIZE, IMAGE_SIZE))
        IMAGES['ocean'] = pg.transform.scale(pg.image.load('./images/ocean.png'), (IMAGE_SIZE, IMAGE_SIZE))
        IMAGES['start'] = pg.transform.scale(pg.image.load('./images/start.png'), (IMAGE_SIZE, IMAGE_SIZE))
        IMAGES['end'] = pg.transform.scale(pg.image.load('./images/end.png'), (IMAGE_SIZE, IMAGE_SIZE))
        IMAGES['human'] = pg.transform.scale(pg.image.load('./images/human.jpg'), (IMAGE_SIZE, IMAGE_SIZE))
